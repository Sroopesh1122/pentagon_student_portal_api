...
public interface BatchTechTrainerRepository extends JpaRepository<BatchTechTrainer, Integer> {
	...
	@Query("SELECT COUNT(btt) = 0 FROM BatchTechTrainer btt WHERE btt.trainer.id = :trainerId AND btt.completed = false AND (:startTime < btt.endTime AND :endTime > btt.startTime)")
	boolean isTrainerAvailable(String trainerId, Double startTime, Double endTime);
